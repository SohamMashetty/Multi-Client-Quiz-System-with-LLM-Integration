...
map<string, vector<Question>> genre_cache;  
...
vector<Question> generateQuiz(const string &genre) {
    ...
    //reserve
    {
        ...
        if (sockfd < 0) return qs;

        struct hostent *server = gethostbyname("192.168.50.142");
        if (!server) { close(sockfd); return qs; }
    ...
    if (start == string::npos || end == string::npos) return qs;
...
void handle_client(int client_socket) {
    ...
    while (client_is_alive.load()) {
        ...
        cout << "[SERVER] Genre request: " << genre << endl;

        {
            // reset timer so keep alive doesn’t kill the client
            lock_guard<mutex> lg(last_msg_mutex);
            last_message_time = chrono::steady_clock::now();
        }
        ...
        {
            lock_guard<mutex> lock(quiz_mutex);
            if (genre_cache.count(genre)) {
                cout << "[CACHE] Cache hit for genre " << genre << endl;
                qs = genre_cache[genre];
                send_msg(client_socket, "INFO|Cached questions used for genre " + genre);
            } else {
                cout << "[CACHE] Cache miss for genre " << genre << " → fetching from LLM\n";
                qs = generateQuiz(genre);
                genre_cache[genre] = qs;
                send_msg(client_socket, "INFO|Fresh questions generated for genre " + genre);
            }
        }
        ...
        if (!qs.empty()) {
            ...
            send_msg(client_socket, "ERROR|Failed to generate questions. Try again.");
        }
    }
